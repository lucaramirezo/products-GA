...
export interface PricedProductRow {
  ...
  override: { mult:number; number_of_layers:number; source:'product'|'category' } | null;
  activePricing: {
    base_per_sqft:number; base_total:number; ink_add:number; lam_add:number; cut_add:number; addons_total:number; final:number;
  };
...
export function buildPricedProductRow(args:{ product:Product; tiers:Tier[]; params:PriceParams; categoryRule?:CategoryRule; }):PricedProductRow {
  ...
  if(breakdown.effective.sources.some(s=>s==='product.mult') || breakdown.effective.sources.some(s=>s==='category.mult')){
    ...
    override = { mult: breakdown.effective.mult, number_of_layers: breakdown.effective.number_of_layers, source: isProduct ? 'product' : 'category' };
  }
