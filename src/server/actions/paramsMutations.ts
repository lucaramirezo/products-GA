...
export async function updateParams(patch: Partial<PriceParams>): Promise<PriceParams> {
  const db = getDb();
    ...
    if (patch.cut_unit !== undefined) updateData.cutUnit = patch.cut_unit;
    if (patch.rounding_step !== undefined) updateData.roundingStep = patch.rounding_step.toString();
    if (patch.min_pvp_global !== undefined) updateData.minPvpGlobal = patch.min_pvp_global?.toString();
    ...
    if (currentParams) {
      ...
      const insertData = {
        ...
        cutUnit: patch.cut_unit ?? 'per_sqft',
        roundingStep: patch.rounding_step?.toString() ?? '0.05',
        minPvpGlobal: patch.min_pvp_global?.toString(),
    ...
    const result: PriceParams = {
      ...
      cut_unit: updatedParams.cutUnit as "per_sqft" | "per_sheet",
      rounding_step: Number(updatedParams.roundingStep),
      min_pvp_global: updatedParams.minPvpGlobal ? Number(updatedParams.minPvpGlobal) : 0,
